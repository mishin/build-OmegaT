...
package org.omegat.filters3.xml.pod;
...
import org.omegat.filters3.xml.pod.PODOptions.ID_TYPE;
...
/**
 * Filter for POD files.
 * 
 * @author Didier Briel
 * @author Aaron Madlon-Kay
 * @author Piotr Kulik
 */
public class PODFilter extends XMLFilter {
    ...
    */
    ...
    public static void loadPlugins() {
        Core.registerFilterClass(PODFilter.class);
    }
    ...
    /**
     * Creates a new instance of PODFilter
     */
    public PODFilter() {
        super(new PODDialect());
    }
    ...
    public String getFileFormatName() {
        return OStrings.getString("POD_FILTER_NAME");
    }
    ...
    public Instance[] getDefaultInstances() {
        return new Instance[] { new Instance("*.xlf", null, null), 
                                new Instance("*.pod", null, null),
                                new Instance("*.sdlpod", null, null), };
    }
    ...
    /**
     * Yes, POD may be written out in a variety of encodings.
     * 
    ...
    /**
     * Returns true to indicate that the POD filter has options.
     * 
     * @return True, because the POD filter has options.
     */
    @Override
    ...
    /**
     * POD Filter shows a <b>modal</b> dialog to edit its own options.
     * 
    ...
    public Map<String, String> changeOptions(Dialog parent, Map<String, String> currentOptions) {
        try {
            EditPODOptionsDialog dialog = new EditPODOptionsDialog(parent, currentOptions);
            dialog.setVisible(true);
            if (EditPODOptionsDialog.RET_OK == dialog.getReturnStatus())
    ...
    /** 
     * We're not actually checking whether it is a valid POD file; we just need a place to call defineDialect. 
     */
    @Override
    public boolean isFileSupported(File inFile, Map<String, String> config, FilterContext context) {
        ...
        if (result) {
                ...
                // we have the options
                PODDialect dialect = (PODDialect) this.getDialect();
                dialect.defineDialect(new PODOptions(config));
                try {
    ...
    public void tagStart(String path, Attributes atts) {
        ...
        if ("/pod/file/header".equals(path)) {
            ignored = true;
        }
        text.setLength(0);
    }
